...
function buildBannerSection(movie) {
  ...
  bannerCont.style.backgroundImage = `url('/bg-vscode/6.jpg')`;

  const div = document.createElement("div");

  div.innerHTML = `
            <h2 class="banner__title">Still The Same Feeling</h2>
            <p class="banner__overview">For many years, their love story continues to develop beautifully. Every trial they face actually becomes fertilizer that fertilizes the seed of their love, so that it becomes stronger and bears sweet fruit.</p>
...
function buildMoviesSection(list, categoryName) {
  ...
  // Lakukan penyesuaian seperti looping atau pengambilan random
  const moviesListHTML = list
    .map((item, index) => {
      return `
          <div class="movie-item" onmouseenter="searchMovieTrailer('${item.title}', 'yt${item.id}')">
              <img class="move-item-img" src="${customImages[index]}" alt="${item.title}" />
